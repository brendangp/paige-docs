...
### Send a test template to your linked phone

Once you've linked a phone, the Paige Dev card has a **Send test template** button. Click it and Paige delivers a pre-built Meta template — with an embedded flow-trigger button — straight to your linked WhatsApp number, so you can verify template rendering and flow buttons end-to-end without doing a full deploy.
...
<Tip>
  Use **Send test template** when you want to confirm that a flow opens correctly from a tappable button — it bypasses the need to start an outbound conversation manually.
</Tip>
