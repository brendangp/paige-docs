...
<Note>
  A scheduled task runs with the same setup as the rest of your bot. Your [secrets](/guides/secrets) are available, and so are your [integrations](/integrations) — code that talks to Airtable or Slack through an integration works on a schedule exactly as it does when a customer messages you. A nightly Airtable sync or a daily Slack summary is a perfectly good thing to build.
</Note>
