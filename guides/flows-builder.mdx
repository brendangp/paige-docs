...
## Preview flows inside Paige

You can preview every flow in your project from the **Flows** tab — open a flow on the left, then click **Try it** on the detail panel to load an interactive simulation in the right pane. **Inspect** and **Debug** modes are available alongside **Try it** for stepping through screens and validating the JSON.

Preview works even if you haven't connected your own WhatsApp number yet. Paige dual-publishes every flow to the shared Paige Dev WhatsApp Business Account, so the preview falls back to the Paige Dev copy whenever your own copy isn't available. You don't have to do anything different — just open a flow and click **Try it**.
