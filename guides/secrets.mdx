...
<PageDemo src="/assets/videos/secrets.mp4" poster="/assets/videos/secrets.poster.jpg" />
...
<Tip>
  For **Airtable** and **Slack**, don't store a raw token here — use [Connectors](/integrations) instead. Connectors handle the sign-in for you and keep the credential out of your hands entirely. Secrets are the right tool for everything else.
</Tip>
...
<AccordionGroup>
  <Accordion title="OpenAI API key" icon="bot">
  ...
  <Accordion title="CRM API credentials" icon="contact">
  ...
  <Accordion title="Third-party service credentials" icon="plug">
  ...
  <Accordion title="Webhook verification tokens" icon="webhook">
