...
<CardGroup cols={2}>
  ...
  <Card title="Broadcasts" icon="megaphone">
    Send a message to many contacts at once, with reusable audience segments and opt-out handling. Import contacts in bulk to build an audience.
  </Card>
  <Card title="Storage and media" icon="hard-drive">
    The images and documents your bot sends, plus anything customers send in that you choose to keep.
  </Card>
  <Card title="Connectors" icon="plug">
    Per-project connections to Airtable and Slack, so your bot can read and write there without you handling a token.
  </Card>
</CardGroup>

Most of these live under the project's [**Tools** tab](/guides/tools).
...
You can update a project's name and settings at any time from the project settings page.

WhatsApp Business profile details — including your display name and profile photo — are managed in **Settings → WhatsApp**, and only become available once you've connected a WhatsApp number to the project.
...
Project suspension is driven by billing — there is no manual "suspend" button. A suspended project is taken offline (the runtime stops responding to WhatsApp messages) but its data, bot code, database, flows, and conversation history are preserved.

A project becomes suspended in two situations:
...
See [Billing](/guides/billing) for the full billing model and how to manage your plan.
