...
<CardGroup cols={2}>
  ...
  <Card title="Connector tools" icon="plug">
    Look up how [Airtable and Slack](/integrations) really work before writing code against them — including your own tables and fields.
  </Card>
...
<Note>
  A document you upload as a **template** deliberately stays out of the knowledge base — see [Documents](/guides/documents). That's the difference between a file your bot *answers from* and one it *fills in*.
</Note>

### Documents it can fill in and send

If you've uploaded a template, the agent can see it — the slug, the filename, and the format — and will use it rather than inventing a document from scratch.

Ask it to fill one in and it reads the file **once**, then stops and shows you a card listing the blanks it found. You rename, drop, or confirm them, and only then does it write the code — against exactly the fields you approved.

That pause is the point. The list it writes code against is a decision you made, not one it guessed, and the code that comes out fills in the same blanks the same way every time, with no AI at send time. See [Documents](/guides/documents).
...
<Card title="Prompting tips" icon="lightbulb" href="/guides/prompting">
  Describe outcomes not implementation, iterate in small steps, know when the agent edits code versus builds a Flow, and understand why a fresh project feels different. See [Prompting the Code Agent well](/guides/prompting).
</Card>
